"""
GraphService — research knowledge graph builder.

Takes ranked search results and produces a React Flow-compatible knowledge
graph with extracted entities, topics, and their relationships.

Pipeline:
    results → entity extraction (spaCy NER → regex fallback)
            → topic extraction (noun chunks)
...
# ── spaCy: primary extractor; graceful fallback when unavailable ──────────────
try:
    import spacy
...
# spaCy NER label → semantic node type
...
# Corporate / product suffixes that indicate an org regardless of spaCy's label
...
# Tokens that mark an entity as a technology/product rather than a person
...
def _override_entity_type(label: str, spacy_type: str) -> str:
    """
    Correct common spaCy misclassifications for tech/AI entities.

    Rules (applied in order — first match wins):
      1. Label ends with a corporate/org suffix → organization
      2. Label contains a standalone "AI" token → organization
         (e.g. "Scale AI", "Devin AI", "OpenAI" — companies, not people)
      3. Any word in the label is a known tech token → technology
      4. Fall through: keep spaCy's original classification
    """
    lower = label.lower()
    ...
    # "AI" as a standalone word (e.g. "Scale AI") or fused suffix (e.g. "OpenAI")
...
# Entity types to skip (too noisy for a knowledge graph)
_SKIP_LABELS: frozenset[str] = frozenset({
    "DATE", "TIME", "PERCENT", "MONEY", "QUANTITY",
    "ORDINAL", "CARDINAL",
})

# Suggested fill colours per node type (frontend hint)
_TYPE_COLOR: dict[str, str] = {
    "query":        "#6366f1",   # indigo  — root
    "topic":        "#06b6d4",   # cyan    — noun-chunk topics
    ...
    "platform":     "#334155",   # dark-slate — source platform
    "facility":     "#a3a3a3",   # neutral
    "unknown":      "#9ca3af",   # gray
}
...
# Minimum character length for a valid entity label
_MIN_LABEL_LEN = 3

# Dependency verb relations to surface as named edge labels
_RICH_DEPS = frozenset({
    "nsubj", "nsubjpass", "dobj", "pobj", "attr", "appos",
})

# Words that should never become graph nodes — covers sentence-start capitals,
# common pronouns, determiners, auxiliary verbs, and generic nouns.
_GRAPH_STOPWORDS: frozenset[str] = frozenset({
    ...
    # Auxiliary / common verbs at sentence start
    ...
    # Prepositions / conjunctions (can appear capitalised mid-sentence)
...
# Lowercase mirror for bigram topic filtering
...
# ─────────────────────────────── regex fallback entity patterns ───────────────

_FALLBACK_PATTERNS: list[tuple[re.Pattern, str]] = [
    # Multi-word capitalized proper nouns (≥2 words, e.g. "React Flow", "Bell Labs")
    ...
    # Version strings next to product names (e.g. "Python 3", "GPT-4")
...
class GraphService:
    """
    Builds a React Flow-compatible knowledge graph from ranked search results.

    Usage:
        gs    = GraphService()
        graph = gs.build_graph(ranked_results, query="transformer attention")
        # graph["nodes"] and graph["edges"] → feed directly to <ReactFlow />
    ...
    Node schema (React Flow):
        {
            ...
            "data": {
                ...
                "nodeType":   str,    # semantic type (topic/person/org/…)
                "count":      int,    # frequency in results
                "platforms":  list,   # platforms where entity appears
                "urls":       list,   # source URLs
                "color":      str,    # suggested hex colour
            }
        }
    ...
    Edge schema (React Flow):
        {
            ...
            "source":   str,          # node id
            "target":   str,          # node id
            "type":     str,          # smoothstep / default / straight / step
            "label":    str,          # relation label
            "animated": bool,
            "data": {
                "relation":    str,
                "weight":      int,   # co-occurrence count
                "strokeWidth": int,   # CSS hint for React Flow styling
            }
        }
    """
    ...
    def __init__(
        ...
        """
        Args:
            min_entity_freq: Only include entities that appear in at least
                             this many results. Raise to 2+ for cleaner graphs
                             with large result sets.
            max_nodes:       Hard cap on total non-platform nodes.
            model:           spaCy model name (ignored when spaCy unavailable).
        """
    ...
    def build_graph(
        ...
        """
        Build the full knowledge graph from ranked results.

        Args:
            results: Output of ScorerService.score() (or any AnalyzerService
                     result list).  Each item needs title, snippet, url,
                     platform at minimum.
            query:   Original search query; becomes the root node.
        ...
        # ── 2. Frequency filtering + deduplication ────────────────────────────
        ...
        # Keep only the top-N by frequency
        top_entities = sorted(
            combined.values(),
        ...
        # ── 5. Layout (radial, frequency-tiered) ─────────────────────────────
    ...
    def _extract_entities(
        ...
        """
        Run NER over title + snippet for every result.

        Returns a flat list of raw entity records:
            {"text": str, "entity_type": str, "result_url": str,
             "platform": str, "in_title": bool}
        """
        if self._nlp:
    ...
    def _extract_with_spacy(
        ...
        for result, doc in zip(results, self._nlp.pipe(texts, batch_size=32)):  # type: ignore[union-attr]
            ...
            for pattern, entity_type in _FALLBACK_PATTERNS:
                for match in pattern.finditer(text):
                    ...
                    if label in _GRAPH_STOPWORDS or norm in seen_in_result:
                        continue
                    # Drop single-word matches whose first word is a stopword
    ...
    def _extract_topics(
        ...
        """
        Extract multi-word noun chunks as 'topic' nodes.
        Only runs when spaCy is available (noun chunks need a dependency parse).
        Falls back to bigram extraction from titles.
        """
        if self._nlp:
    ...
    def _topics_with_spacy(
        ...
        records: list[dict[str, Any]] = []
        for result, doc in zip(results, self._nlp.pipe(title_texts, batch_size=32)):  # type: ignore[union-attr]
            for chunk in doc.noun_chunks:
                label = chunk.text.strip()
                # Only multi-word chunks; skip single common words
    ...
    def _topics_from_titles(
        ...
        """Bigram extraction from titles when spaCy is unavailable."""
        records: list[dict[str, Any]] = []
        for result in results:
    ...
    def _deduplicate(
        ...
        """
        Collapse records with the same normalised text into a single entry,
        counting frequency and collecting source URLs + platforms.
        """
    ...
    def _merge_maps(
        ...
        """
        Merge entity and topic maps, preferring a named-entity type over 'topic'
        when the same text appears in both.
        """
        merged = dict(topic_map)
        for norm, entry in entity_map.items():
            if norm in merged:
                # Named entity wins; accumulate counts
    ...
    def _build_nodes(
        ...
        """
        Build React Flow node dicts.

        Returns:
            (node_list, node_lookup)
            node_lookup: canonical_text_lower → node_id
        """
        ...
        for entry in top_entities:
            ...
            nodes.append({
                "id":   entry["id"],
                "type": rf_type,
                "position": {"x": 0, "y": 0},   # filled in by layout
                "data": {
                    "label":     entry["label"],
                    "nodeType":  etype,
                    "count":     entry["count"],
    ...
    def _build_edges(
        ...
        # ── a. query → entities (top title-hit entities get animated edges) ───
        query_id    = "query-root"
        ...
        # ── c. co-occurrence: entities sharing the same result ────────────────
        #    Group records by result URL, then pair entities within each group
        ...
        for url, eids in url_to_entities.items():
            unique = list(dict.fromkeys(eids))   # preserve order, deduplicate
            for i in range(len(unique)):
                for j in range(i + 1, len(unique)):
                    a, b = unique[i], unique[j]
                    # Normalise direction: smaller id first for dedup
                    key = (a, b) if a < b else (b, a)
                    edge_weights[key] += 1
                    edge_relations.setdefault(key, "co_occurs_with")

        # ── d. spaCy dependency relations (enriched labels) ──────────────────
        if self._nlp:
            ...
            for (src, tgt, relation) in dep_edges:
                ...
                edge_relations[key] = relation   # overwrite generic label
    ...
    def _extract_dep_relations(
        ...
        """
        Use spaCy dependency parse to find verb-mediated entity relations.
        Returns [(source_node_id, target_node_id, relation_verb), ...]
        """
        found: list[tuple[str, str, str]] = []
        texts  = [f"{r.get('title', '')}. {r.get('snippet', '')}" for r in results]

        for doc in self._nlp.pipe(texts, batch_size=32):  # type: ignore[union-attr]
            for token in doc:
                ...
                for subj in subjects[:1]:
                    for obj in objects[:1]:
                        ...
                        if sid and oid and sid != oid:
                            ...
                            found.append((sid, oid, verb[:20]))   # truncate long verbs
        return found
    ...
    def _apply_layout(
        ...
        """
        Radial tiered layout — assigned in-place.

        Tier  0  (query root)  : origin (0, 0)
        Tier  1  (count ≥ 5)   : inner ring   r = 280
        Tier  2  (count 2-4)   : middle ring  r = 500
        Tier  3  (count = 1)   : outer ring   r = 700
        Platforms               : fixed ring   r = 900
        """
        query_id    = "query-root"
        ...
        # Sort entities into frequency tiers
...
def _short_hash(text: str) -> str:
    """8-char deterministic ID fragment from text."""
    return hashlib.md5(text.encode()).hexdigest()[:8]


def _rf_node_type(entity_type: str) -> str:
    """Map semantic type to a React Flow custom node type name."""
    mapping = {
        "query":        "queryNode",
        "topic":        "topicNode",
